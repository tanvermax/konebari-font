...
export default function AdminOrderTrack() {
    ...
    const [localUpdating, setLocalUpdating] = useState<{ [key: string]: boolean }>({});

    const ordersList = orderResponse?.data || [];

    const handleStatusChange = async (orderId: string, newStatus: string, isDelete: boolean = false) => {
        ...
        try {
            ...
            if (res.success) {
                toast.success(isDelete ? "Logistics Reset" : `Order #${orderId.slice(-4)} Synced`);
        ...
        <div className="space-y-6 p-4 max-w-[1600px] mx-auto">
            ...
            <header className="flex items-end justify-between bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                <div>
                    ...
                    <p className="text-slate-400 text-sm font-medium">Real-time logistics management</p>
                </div>
                <div className="flex gap-4">
                    <div className="text-right">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active Orders</p>
                        <p className="text-2xl font-black text-slate-900">{ordersList.length}</p>
                    </div>
                </div>
            </header>
                ...
                <Table>
                    <TableHeader className="bg-slate-50/50">
                        <TableRow className="hover:bg-transparent border-none">
                            <TableHead className="font-bold text-slate-500 py-4 pl-6">Order</TableHead>
                            <TableHead className="font-bold text-slate-500">Customer</TableHead>
                            <TableHead className="font-bold text-slate-500">Inventory</TableHead>
                            <TableHead className="font-bold text-slate-500">Logistics</TableHead>
                            <TableHead className="text-right font-bold text-slate-500 pr-8">Control</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                                ...
                                >
                                    ...
                                    <TableCell className="pl-6 py-6 align-top">
                                        <div className="flex flex-col gap-2">
                                            <span className="font-mono text-[11px] font-bold text-slate-400">#{order._id.slice(-8)}</span>
                                            <Badge className={`w-fit shadow-none border ${getStatusColor(order.status)}`}>
                                                {order.status}
                                            </Badge>
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="align-top py-6">
                                        <div className="flex flex-col gap-1 max-w-[180px]">
                                            <span className="font-bold text-slate-800 text-sm truncate uppercase">{order.shippingAddress?.name}</span>
                                            <div className="flex items-center gap-1.5 text-slate-500">
                                                <Phone className="h-3 w-3" />
                                                <span className="text-[11px] font-medium">{order.shippingAddress?.phone}</span>
                                            </div>
                                    ...
                                    <TableCell className="align-top py-6">
                                        <div className="space-y-3">
                                            {order.orderedItems?.slice(0, 2).map((item: any, i: number) => (
                                                <div key={i} className="flex items-center gap-3 group/item">
                                                    <div className="h-9 w-9 rounded-lg border bg-slate-50 overflow-hidden shrink-0">
                                                        <img src={item.product?.images} className="object-cover h-full w-full" alt="" />
                                                    </div>
                                                    <div className="min-w-0">
                                                        <p className="text-[11px] font-bold text-slate-700 truncate w-[160px] group-hover/item:w-[250px] transition-all">
                                                            {item.product?.["*Product Name(English)"]}
                                                        </p>
                                                        <p className="text-[10px] text-orange-600 font-bold">Qty {item.quantity}</p>
                                                    </div>
                                                </div>
                                            ))}
                                    ...
                                    <TableCell className="align-top py-6">
                                        <div className="w-[180px]">
                                            {(!order.courierName || order.courierName === "N/A" || editingLogistics[order._id]) ? (
                                                <div className="flex flex-col gap-1.5 animate-in fade-in slide-in-from-left-2">
                                                    <Input 
                                                        ...
                                                        defaultValue={order.courierName !== "N/A" ? order.courierName : ""}
                                                        onChange={(e) => setCourierName({...courierName, [order._id]: e.target.value})}
                                                    />
                                                    <Input 
                                                        placeholder="ID" 
                                                        className="h-7 text-[10px] focus-visible:ring-orange-500"
                                                        defaultValue={order.trackingId !== "N/A" ? order.trackingId : ""}
                                                ...
                                                <div className="relative group/logistic p-2 rounded-xl border border-transparent hover:border-slate-100 hover:bg-slate-50 transition-all">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{order.courierName}</span>
                                                        <div className="flex gap-2 opacity-0 group-hover/logistic:opacity-100 transition-opacity">
                                                            <button onClick={() => setEditingLogistics({...editingLogistics, [order._id]: true})} className="text-blue-500 hover:scale-110"><Edit3 className="h-3 w-3" /></button>
                                                            <button onClick={() => handleStatusChange(order._id, order.status, true)} className="text-rose-500 hover:scale-110"><Trash2 className="h-3 w-3" /></button>
                                                        </div>
                                                    </div>
                                                    <p className="text-[11px] font-bold text-slate-700 truncate">{order.trackingId}</p>
                                                </div>
                                            )}
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="text-right py-6 pr-8 align-top">
                                        <div className="inline-flex flex-col gap-2">
                                            ...
                                            >
                                                ...
                                                <SelectContent>
                                                    {['Pending', 'Shipped', 'Completed', 'Cancelled'].map(s => (
                                                        <SelectItem key={s} value={s} className="text-[11px] font-bold">{s}</SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                            <Button
                                            ...
                                            >
                                                {localUpdating[order._id] ? <Loader2 className="h-3 w-3 animate-spin" /> : "Sync Logistics"}
                                            </Button>
                                        </div>
                                    </TableCell>
                                </motion.tr>
